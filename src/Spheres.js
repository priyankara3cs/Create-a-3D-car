...
function Sphere({ color }) {
  ...
  useFrame(
    (state, delta) => {
      ...
      sphere.current.position.set(position.x, position.y, newZ);
  ...
  return (
    <mesh ref={sphere} rotation-x={Math.PI * 0.9} scale={scale} castShadow>
...
export function Spheres() {
  ...
  return (
    <>
      {arr.map((e, i) => (
        <Sphere key={i} color={i % 2 === 0 ? [0.4, 0.1, 0.1] : [0.05, 0.15, 0.4]} />
      ))}
    </>
  );
}
